...
export default function Pricing() {
    ...
    return (
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-[2px] rounded-md overflow-hidden mx-0 lg:mx-36' id='pricing-grid'>
            ...
            <div className='row-span-3 bg-[#fec5baCA] plan-grid-ele order-5 sm:order-none'></div>
            <div className='bg-[#fdcdc4CA] plan-grid-ele pt-[30px] pb-[60px] order-2 sm:order-none'>
                <span className='flex justify-center items-end mb-3'>
                    <h4 className='text-2xl mr-[4px] font-bold'>{dict.pricing.monthly.cost}</h4>
                ...
                {dict.pricing.monthly.included.map((ele, i, arr) => (
                    <p key={ele} className={clsx(i === 0 && 'mt-8', i + 1 !== arr.length && 'border-b', 'text-sm border-[#1f2d3d20] py-2 mx-12 font-semibold')}>
                        {ele}
                    </p>
                ))}
            </div>
            <div className='bg-[#fdcdc4CA] plan-grid-ele py-[150px] order-4 sm:order-none'></div>
            <div className='col-span-1 sm:col-span-2 bg-[#fec5baCA] plan-grid-ele py-[90px] order-6 sm:order-none'></div>
        </div>
    );
}
